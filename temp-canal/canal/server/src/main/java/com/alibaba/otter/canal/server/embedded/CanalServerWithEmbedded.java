...
public class CanalServerWithEmbedded extends AbstractCanalLifeCycle implements CanalServer, CanalService {
    ...
    /***
     * @Description: 启动一个Canal服务
     *
     * - 启动promethos 服务
     * - 声明String,CanalInstance延迟计算的map，初始化逻辑为 canalInstanceGenerator.generate(destination)
     *
     *
     * @Param: []
     * @return: void
    ...
    public void start() {
        if (!isStart()) {
            ...
            //- load promethos service
            loadCanalMetrics();
            metrics.setServerPort(metricsPort);
            metrics.initialize();

            //- 懒加载canalInstance，最终会初始化spring配置文件中的bean.
